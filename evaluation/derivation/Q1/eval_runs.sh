...
for size in "${sizes[@]}"; do
    for model in "${models_ollama[@]}"; do
        ...
        python evaluation/derivation/Q1/eval_scripts/pz_q1_eval.py -s $size -m $model -p ollama
        echo ""
        echo "Evaluating BlendSQL with -s $size and -m $model"
        python evaluation/derivation/Q1/eval_scripts/blendsql_q1_eval.py -s $size -m $model -p ollama
        echo ""
    done
    ...
    for model in "${models_transformers[@]}"; do
        echo "Evaluating BlendSQL with -s $size and -m $model"
        python evaluation/derivation/Q1/eval_scripts/blendsql_q1_eval.py -s $size -m $model -p transformers
        echo ""
    done
