...
for size in "${sizes[@]}"; do
    ...
    for model in "${models_vllm[@]}"; do
        ...
        python evaluation/derivation/Q3/eval_scripts/blendsql_q3_eval.py -s $size -m $model -p vllm
        echo ""
    done

    for model in "${models_transformers[@]}"; do
        echo "Evaluating BlendSQL -s $size and -m $model (Transformers)"
        python evaluation/derivation/Q3/eval_scripts/blendsql_q3_eval.py -s $size -m $model -p transformers
        echo ""
    done
done