...
for size in "${sizes[@]}"; do
    for model in "${models_ollama[@]}"; do
        ...
        python evaluation/projection/Q4/eval_scripts/blendsql_q4_eval.py -s $size -m $model -p ollama
        echo ""
    done
    ...
    for model in "${models_vllm[@]}"; do
        ...
        python evaluation/projection/Q4/eval_scripts/blendsql_q4_eval.py -s $size -m $model -p vllm
        echo ""
    done

    for model in "${models_transformers[@]}"; do
        echo "Evaluating BlendSQL with -s $size -m $model (Transformers)"
        python evaluation/projection/Q4/eval_scripts/blendsql_q4_eval.py -s $size -m $model -p transformers
    done
